...
for k in range(hop):#あるクラスタ内に存在するノードに対して，CHからのホップ数を全て出力
    for i in range(N):
        if hop_frag_0[i]==1:
            for j in range(N):
...
for i in range(N):#上と統合すると、プロットが被る
    if CH_num[i]==i:#CH
        plt.plot(x[i],y[i],markeredgecolor='black',marker='*',markersize=20)
        #ax.annotate(str(hop_count[i]),(x[i],y[i]))#GNからのホップ数を出力
...
for i in range(N):#所属クラスタが異なる隣接ノード
    for j in range(N):
        if link[i][j]==1 and CH_num[i]!=CH_num[j]:
            plt.plot(x[i],y[i],marker='o',markeredgecolor='black')#GNを枠線で囲む
...
plt.xlabel('x-coordinate')
plt.ylabel('y-coordinate')
plt.savefig('/Users/kawasakirio/Desktop/M1result/0702/'+'dec='+str(q_dec)+'t='+str(time)+'sigma='+str(sigma)+'sd='+str(sd)+'.pdf')#語尾に'.pdf'が必要
plt.cla()
'''
